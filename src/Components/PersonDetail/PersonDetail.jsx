...
import { getContactDetails, getCareer } from '../../utils/api';
...
const PersonDetail = ({ person }) => {
  ...
  useEffect(async () => {
    const dataCareer = await getCareer();
    const data = await getContactDetails();
    setList(data?.results || []);
    setCareer(dataCareer?.results.sort((a, b) => new Date(b.date) - new Date(a.date))[0] || {});
  }, []);
    ...
    <div className={styles.container}>
      <div className={styles.wraper}>
        ...
        <div>
          ...
          <div className={styles.main}>{career.position}</div>
        </div>
      </div>
